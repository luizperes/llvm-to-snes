...
unsigned
SNESMCCodeEmitter::loadStorePostEncoder(const MCInst &MI, unsigned EncodedValue,
...
unsigned
SNESMCCodeEmitter::encodeRelCondBrTarget(const MCInst &MI, unsigned OpNo,
  ...
  SNES::fixups::adjustBranchTarget(target);
  return target;
}

unsigned SNESMCCodeEmitter::encodeLDSTPtrReg(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
...
unsigned SNESMCCodeEmitter::encodeMemri(const MCInst &MI, unsigned OpNo,
                                       SmallVectorImpl<MCFixup> &Fixups,
                                       const MCSubtargetInfo &STI) const {
  ...
  if (OffsetOp.isImm()) {
    ...
    Fixups.push_back(MCFixup::create(0, OffsetOp.getExpr(),
                     MCFixupKind(SNES::fixup_6), MI.getLoc()));
  } else {
...
unsigned SNESMCCodeEmitter::encodeComplement(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
...
unsigned SNESMCCodeEmitter::encodeImm(const MCInst &MI, unsigned OpNo,
                                     SmallVectorImpl<MCFixup> &Fixups,
                                     const MCSubtargetInfo &STI) const {
  auto MO = MI.getOperand(OpNo);

  if (MO.isExpr()) {
    if (isa<SNESMCExpr>(MO.getExpr())) {
      // If the expression is already an SNESMCExpr (i.e. a lo8(symbol),
...
unsigned SNESMCCodeEmitter::encodeCallTarget(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
  auto MO = MI.getOperand(OpNo);

  if (MO.isExpr()) {
    MCFixupKind FixupKind = static_cast<MCFixupKind>(SNES::fixup_call);
...
unsigned SNESMCCodeEmitter::getExprOpValue(const MCExpr *Expr,
                                          SmallVectorImpl<MCFixup> &Fixups,
                                          const MCSubtargetInfo &STI) const {
  ...
  if (Kind == MCExpr::Target) {
    SNESMCExpr const *SNESExpr = cast<SNESMCExpr>(Expr);
    int64_t Result;
    if (SNESExpr->evaluateAsConstant(Result)) {
      return Result;
    }

    MCFixupKind FixupKind = static_cast<MCFixupKind>(SNESExpr->getFixupKind());
    Fixups.push_back(MCFixup::create(0, SNESExpr, FixupKind));
    return 0;
  }
...
unsigned SNESMCCodeEmitter::getMachineOpValue(const MCInst &MI,
                                             const MCOperand &MO,
                                             SmallVectorImpl<MCFixup> &Fixups,
                                             const MCSubtargetInfo &STI) const {
...
void SNESMCCodeEmitter::emitInstruction(uint64_t Val, unsigned Size,
