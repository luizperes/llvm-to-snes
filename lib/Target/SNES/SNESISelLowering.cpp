...
SNESTargetLowering::SNESTargetLowering(SNESTargetMachine &tm)
  ...
  setStackPointerRegisterToSaveRestore(SNES::SP);
  ...
  // Do not use MUL. The SNES instructions are closer to SMUL_LOHI &co.
  ...
  // TODO: check what to use on the code below
  /*
  // Several of the runtime library functions use a special calling conv
  setLibcallCallingConv(RTLIB::SDIVREM_I8, CallingConv::SNES_BUILTIN);
  setLibcallCallingConv(RTLIB::SDIVREM_I16, CallingConv::SNES_BUILTIN);
  setLibcallCallingConv(RTLIB::UDIVREM_I8, CallingConv::SNES_BUILTIN);
  setLibcallCallingConv(RTLIB::UDIVREM_I16, CallingConv::SNES_BUILTIN);
  */
...
EVT SNESTargetLowering::getSetCCResultType(const DataLayout &DL, LLVMContext &,
                                          EVT VT) const {
  assert(!VT.isVector() && "No SNES SetCC type for vectors!");
  return MVT::i8;
}

SDValue SNESTargetLowering::LowerShifts(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (!isa<ConstantSDNode>(N->getOperand(1))) {
    ...
    case ISD::SHL:
      return DAG.getNode(SNESISD::LSLLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    case ISD::SRL:
      return DAG.getNode(SNESISD::LSRLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    case ISD::ROTL:
      return DAG.getNode(SNESISD::ROLLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    case ISD::ROTR:
      return DAG.getNode(SNESISD::RORLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    case ISD::SRA:
      return DAG.getNode(SNESISD::ASRLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    }
  }
...
SDValue SNESTargetLowering::LowerDivRem(SDValue Op, SelectionDAG &DAG) const {
...
SDValue SNESTargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  return DAG.getNode(SNESISD::WRAPPER, SDLoc(Op), getPointerTy(DL), Result);
}
...
/// IntCCToSNESCC - Convert a DAG integer condition code to an SNES CC.
static SNESCC::CondCodes intCCToSNESCC(ISD::CondCode CC) {
  switch (CC) {
  default:
...
/// Returns appropriate SNES CMP/CMPC nodes and corresponding condition code for
/// the given operands.
SDValue SNESTargetLowering::getSNESCmp(SDValue LHS, SDValue RHS, ISD::CondCode CC,
                                     SDValue &SNEScc, SelectionDAG &DAG,
                                     SDLoc DL) const {
  SDValue Cmp;
  ...
  case ISD::SETGT: {
    if (const ConstantSDNode *C = dyn_cast<ConstantSDNode>(RHS)) {
      ...
      case -1: {
        ...
        UseTest = true;
        SNEScc = DAG.getConstant(SNESCC::COND_PL, DL, MVT::i8);
        break;
      }
      case 0: {
  ...
  case ISD::SETLT: {
    if (const ConstantSDNode *C = dyn_cast<ConstantSDNode>(RHS)) {
      ...
      case 0: {
        ...
        UseTest = true;
        SNEScc = DAG.getConstant(SNESCC::COND_MI, DL, MVT::i8);
        break;
      }
      }
    }
  ...
  if (VT == MVT::i32) {
    ...
    if (UseTest) {
      ...
      SDValue Top = DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i8, LHShi,
                                DAG.getIntPtrConstant(1, DL));
      Cmp = DAG.getNode(SNESISD::TST, DL, MVT::Glue, Top);
    } else {
      Cmp = DAG.getNode(SNESISD::CMP, DL, MVT::Glue, LHSlo, RHSlo);
      Cmp = DAG.getNode(SNESISD::CMPC, DL, MVT::Glue, LHShi, RHShi, Cmp);
    }
  } else if (VT == MVT::i64) {
    ...
    if (UseTest) {
      ...
      Cmp = DAG.getNode(SNESISD::CMP, DL, MVT::Glue, LHS0, RHS0);
      Cmp = DAG.getNode(SNESISD::CMPC, DL, MVT::Glue, LHS1, RHS1, Cmp);
      Cmp = DAG.getNode(SNESISD::CMPC, DL, MVT::Glue, LHS2, RHS2, Cmp);
      Cmp = DAG.getNode(SNESISD::CMPC, DL, MVT::Glue, LHS3, RHS3, Cmp);
    }
  } else if (VT == MVT::i8 || VT == MVT::i16) {
    if (UseTest) {
      // When using tst we only care about the highest part.
      Cmp = DAG.getNode(SNESISD::TST, DL, MVT::Glue,
                        (VT == MVT::i8)
                            ? LHS
                            : DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i8,
                                          LHS, DAG.getIntPtrConstant(1, DL)));
    } else {
      Cmp = DAG.getNode(SNESISD::CMP, DL, MVT::Glue, LHS, RHS);
    }
  } else {
    llvm_unreachable("Invalid comparison size");
  }

  // When using a test instruction SNEScc is already set.
  if (!UseTest) {
    SNEScc = DAG.getConstant(intCCToSNESCC(CC), DL, MVT::i8);
  }

  return Cmp;
}

SDValue SNESTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue TargetCC;
  SDValue Cmp = getSNESCmp(LHS, RHS, CC, TargetCC, DAG, dl);

  return DAG.getNode(SNESISD::BRCOND, dl, MVT::Other, Chain, Dest, TargetCC,
                     Cmp);
}

SDValue SNESTargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(SNESISD::SELECT_CC, dl, VTs, Ops);
}

SDValue SNESTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
...
SDValue SNESTargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
...
SDValue SNESTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  default:
...
bool SNESTargetLowering::isLegalAddressingMode(const DataLayout &DL,
  ...
  if (isa<PointerType>(Ty) && AS == SNES::ProgramMemory) {
    return false;
  }
...
bool SNESTargetLowering::getPreIndexedAddressParts(SDNode *N, SDValue &Base,
                                                  SDValue &Offset,
  ...
  if (const LoadSDNode *LD = dyn_cast<LoadSDNode>(N)) {
    ...
    if (LD->getExtensionType() != ISD::NON_EXTLOAD)
      return false;
    if (SNES::isProgramMemoryAccess(LD)) {
      return false;
    }
  } else if (const StoreSDNode *ST = dyn_cast<StoreSDNode>(N)) {
    VT = ST->getMemoryVT();
    Op = ST->getBasePtr().getNode();
    if (SNES::isProgramMemoryAccess(ST)) {
      return false;
    }
  } else {
    return false;
...
bool SNESTargetLowering::getPostIndexedAddressParts(SDNode *N, SDNode *Op,
                                                   SDValue &Base,
                                                   SDValue &Offset,
                                                   ISD::MemIndexedMode &AM,
                                                   SelectionDAG &DAG) const {
...
bool SNESTargetLowering::isOffsetFoldingLegal(
...
static void analyzeStandardArguments(TargetLowering::CallLoweringInfo *CLI,
  ...
  if (IsVarArg) {
    ...
    if (IsCall) {
      CCInfo.AnalyzeCallOperands(*Outs, ArgCC_SNES_Vararg);
    } else {
      CCInfo.AnalyzeFormalArguments(*Ins, ArgCC_SNES_Vararg);
    }
    return;
  }
  ...
  for (unsigned i = 0, pos = 0, e = Args.size(); i != e; ++i) {
    ...
    // SNES-GCC does not support zero-sized arguments and so we need not
...
static void analyzeBuiltinArguments(TargetLowering::CallLoweringInfo &CLI,
  ...
  if (FuncName.startswith("__udivmod") || FuncName.startswith("__divmod")) {
    CCInfo.AnalyzeCallOperands(*Outs, ArgCC_SNES_BUILTIN_DIV);
  } else {
...
SDValue SNESTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
                                     SmallVectorImpl<SDValue> &InVals) const {
  ...
  // SNES does not yet support tail call optimization.
  isTailCall = false;
  ...
  if (HasStackArgs) {
    for (AE = AI, AI = ArgLocs.size(); AI != AE; --AI) {
      ...
      SDValue PtrOff = DAG.getNode(
          ...
          DAG.getRegister(SNES::SP, getPointerTy(DAG.getDataLayout())),
  ...
  const SNESTargetMachine &TM = (const SNESTargetMachine &)getTargetMachine();
  ...
  Chain = DAG.getNode(SNESISD::CALL, DL, NodeTys, Ops);
  InFlag = Chain.getValue(1);
...
///
SDValue SNESTargetLowering::LowerCallResult(
  ...
  // TODO: check what to do with the code below
  /*
  if (CallConv != CallingConv::SNES_BUILTIN && RVLocs.size() > 1) {
...
CCAssignFn *SNESTargetLowering::CCAssignFnForReturn(CallingConv::ID CC) const {
  switch (CC) {
  // TODO check the code below
  /*
  case CallingConv::SNES_BUILTIN:
    return RetCC_SNES_BUILTIN;
  */
  default:
    return RetCC_SNES;
  }
...
bool
SNESTargetLowering::CanLowerReturn(CallingConv::ID CallConv,
...
SNESTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  // TODO: check the code below later
  /*
  unsigned RetOpc =
      (CallConv == CallingConv::SNES_INTR || CallConv == CallingConv::SNES_SIGNAL)
          ? SNESISD::RETI_FLAG
          : SNESISD::RET_FLAG;
  */
  unsigned RetOpc = SNESISD::RET_FLAG;
...
MachineBasicBlock *SNESTargetLowering::insertShift(MachineInstr &MI,
  ...
  // cpi N, 0
  // breq RemBB
  BuildMI(BB, dl, TII.get(SNES::CPIRdK)).addReg(ShiftAmtSrcReg).addImm(0);
  BuildMI(BB, dl, TII.get(SNES::BREQk)).addMBB(RemBB);
  ...
  BuildMI(LoopBB, dl, TII.get(SNES::PHI), ShiftReg)
      .addReg(SrcReg)
      .addMBB(BB)
      .addReg(ShiftReg2)
      .addMBB(LoopBB);
  BuildMI(LoopBB, dl, TII.get(SNES::PHI), ShiftAmtReg)
      .addReg(ShiftAmtSrcReg)
      .addMBB(BB)
      .addReg(ShiftAmtReg2)
      .addMBB(LoopBB);
  BuildMI(LoopBB, dl, TII.get(Opc), ShiftReg2).addReg(ShiftReg);
  BuildMI(LoopBB, dl, TII.get(SNES::SUBIRdK), ShiftAmtReg2)
      .addReg(ShiftAmtReg)
      .addImm(1);
  BuildMI(LoopBB, dl, TII.get(SNES::BRNEk)).addMBB(LoopBB);

  // RemBB:
  // DestReg = phi [%SrcReg, BB], [%ShiftReg, LoopBB]
  BuildMI(*RemBB, RemBB->begin(), dl, TII.get(SNES::PHI), DstReg)
      .addReg(SrcReg)
      .addMBB(BB)
      .addReg(ShiftReg2)
      .addMBB(LoopBB);
...
MachineBasicBlock *SNESTargetLowering::insertMul(MachineInstr &MI,
                                                MachineBasicBlock *BB) const {
  const SNESTargetMachine &TM = (const SNESTargetMachine &)getTargetMachine();
  const TargetInstrInfo &TII = *TM.getSubtargetImpl()->getInstrInfo();
...
SNESTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  assert((Opc == SNES::Select16 || Opc == SNES::Select8) &&
         "Unexpected instr type to insert");

  const SNESInstrInfo &TII = (const SNESInstrInfo &)*MI.getParent()
                                ->getParent()
                                ->getSubtarget()
                                .getInstrInfo();
  DebugLoc dl = MI.getDebugLoc();
  ...
  SNESCC::CondCodes CC = (SNESCC::CondCodes)MI.getOperand(3).getImm();
  BuildMI(MBB, dl, TII.getBrCond(CC)).addMBB(trueMBB);
  BuildMI(MBB, dl, TII.get(SNES::RJMPk)).addMBB(falseMBB);
  MBB->addSuccessor(falseMBB);
  MBB->addSuccessor(trueMBB);

  // Unconditionally flow back to the true block
  BuildMI(falseMBB, dl, TII.get(SNES::RJMPk)).addMBB(trueMBB);
  falseMBB->addSuccessor(trueMBB);

  // Set up the Phi node to determine where we came from
  BuildMI(*trueMBB, trueMBB->begin(), dl, TII.get(SNES::PHI), MI.getOperand(0).getReg())
...
SNESTargetLowering::getConstraintType(StringRef Constraint) const {
  if (Constraint.size() == 1) {
    // See http://www.nongnu.org/snes-libc/user-manual/inline_asm.html
    switch (Constraint[0]) {
...
unsigned
SNESTargetLowering::getInlineAsmMemConstraint(StringRef ConstraintCode) const {
...
SNESTargetLowering::ConstraintWeight
SNESTargetLowering::getSingleConstraintMatchWeight(
...
SNESTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
                                                ...
                                                MVT VT) const {
  auto STI = static_cast<const SNESTargetMachine &>(this->getTargetMachine())
                 .getSubtargetImpl();
...
void SNESTargetLowering::LowerAsmOperandForConstraint(SDValue Op,
...
unsigned SNESTargetLowering::getRegisterByName(const char *RegName,
                                              EVT VT,
                                              SelectionDAG &DAG) const {
  unsigned Reg;
