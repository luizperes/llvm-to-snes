...
#define DEBUG_TYPE "snes-asm-parser"

namespace llvm {

/// Parses SNES assembly from a stream.
...
/// An parsed SNES assembly operand.
class SNESOperand : public MCParsedAsmOperand {
  typedef MCParsedAsmOperand Base;
  enum KindTy { k_Immediate, k_Register, k_Token, k_Memri } Kind;

public:
  SNESOperand(StringRef Tok, SMLoc const &S)
      : Base(), Kind(k_Token), Tok(Tok), Start(S), End(S) {}
  SNESOperand(unsigned Reg, SMLoc const &S, SMLoc const &E)
      : Base(), Kind(k_Register), RegImm({Reg, nullptr}), Start(S), End(E) {}
  SNESOperand(MCExpr const *Imm, SMLoc const &S, SMLoc const &E)
      : Base(), Kind(k_Immediate), RegImm({0, Imm}), Start(S), End(E) {}
  SNESOperand(unsigned Reg, MCExpr const *Imm, SMLoc const &S, SMLoc const &E)
...
public:
  ...
  static std::unique_ptr<SNESOperand> CreateToken(StringRef Str, SMLoc S) {
    return make_unique<SNESOperand>(Str, S);
  }

  static std::unique_ptr<SNESOperand> CreateReg(unsigned RegNum, SMLoc S,
                                               SMLoc E) {
    return make_unique<SNESOperand>(RegNum, S, E);
  }

  static std::unique_ptr<SNESOperand> CreateImm(const MCExpr *Val, SMLoc S,
                                               SMLoc E) {
    return make_unique<SNESOperand>(Val, S, E);
  }

  static std::unique_ptr<SNESOperand>
  CreateMemri(unsigned RegNum, const MCExpr *Val, SMLoc S, SMLoc E) {
    return make_unique<SNESOperand>(RegNum, Val, S, E);
  }
...
bool SNESAsmParser::invalidOperand(SMLoc const &Loc,
  ...
  if (ErrorInfo != ~0U) {
    if (ErrorInfo >= Operands.size()) {
      ...
      SNESOperand const &Op = (SNESOperand const &)*Operands[ErrorInfo];
...
bool SNESAsmParser::missingFeature(llvm::SMLoc const &Loc,
                                  uint64_t const &ErrorInfo) {
  return Error(Loc, "instruction requires a CPU feature not currently enabled");
}

bool SNESAsmParser::emit(MCInst &Inst, SMLoc const &Loc, MCStreamer &Out) const {
  Inst.setLoc(Loc);
...
bool SNESAsmParser::MatchAndEmitInstruction(SMLoc Loc, unsigned &Opcode,
...
int SNESAsmParser::parseRegisterName(unsigned (*matchFn)(StringRef)) {
  StringRef Name = Parser.getTok().getString();

  int RegNum = matchFn(Name);

  // GCC supports case insensitive register names. Some of the SNES registers
...
int SNESAsmParser::parseRegisterName() {
...
int SNESAsmParser::parseRegister() {
  int RegNum = SNES::NoRegister;
...
bool SNESAsmParser::tryParseRegisterOperand(OperandVector &Operands) {
  int RegNo = parseRegister();

  if (RegNo == SNES::NoRegister)
    return true;

  AsmToken const &T = Parser.getTok();
  Operands.push_back(SNESOperand::CreateReg(RegNo, T.getLoc(), T.getEndLoc()));
  Parser.Lex(); // Eat register token.

  return false;
}

bool SNESAsmParser::tryParseExpression(OperandVector &Operands) {
  ...
  Operands.push_back(SNESOperand::CreateImm(Expression, S, E));
  return false;
}

bool SNESAsmParser::tryParseRelocExpression(OperandVector &Operands) {
  bool isNegated = false;
  SNESMCExpr::VariantKind ModifierKind = SNESMCExpr::VK_SNES_None;
  ...
  ModifierKind = SNESMCExpr::getKindByName(ModifierName.str().c_str());

  if (ModifierKind != SNESMCExpr::VK_SNES_None) {
    Parser.Lex();
  ...
  MCExpr const *Expression = SNESMCExpr::create(ModifierKind, InnerExpression,
...
bool SNESAsmParser::parseOperand(OperandVector &Operands) {
  ...
  case AsmToken::Minus: {
    ...
    Operands.push_back(SNESOperand::CreateToken(Parser.getTok().getString(),
...
SNESAsmParser::parseMemriOperand(OperandVector &Operands) {
  ...
  Operands.push_back(SNESOperand::CreateMemri(RegNo, Expression, S, E));
...
bool SNESAsmParser::ParseRegister(unsigned &RegNo, SMLoc &StartLoc,
  ...
  return (RegNo == SNES::NoRegister);
}

void SNESAsmParser::eatComma() {
...
bool SNESAsmParser::ParseInstruction(ParseInstructionInfo &Info,
                                    StringRef Mnemonic, SMLoc NameLoc,
                                    OperandVector &Operands) {
  Operands.push_back(SNESOperand::CreateToken(Mnemonic, NameLoc));
...
bool SNESAsmParser::ParseDirective(llvm::AsmToken DirectiveID) { return true; }

extern "C" void LLVMInitializeSNESAsmParser() {
  RegisterMCAsmParser<SNESAsmParser> X(getTheSNESTarget());
}
...
// Uses enums defined in SNESGenAsmMatcher.inc
unsigned SNESAsmParser::validateTargetOperandClass(MCParsedAsmOperand &AsmOp,
                                                  unsigned ExpectedKind) {
  SNESOperand &Op = static_cast<SNESOperand &>(AsmOp);
