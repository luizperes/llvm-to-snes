...
//===----------------------------------------------------------------------===//
//
// This file defines an instruction selector for the SNES target.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "snes-isel"

namespace llvm {

/// Lowers LLVM IR (in DAG form) to SNES MC instructions (in DAG form).
class SNESDAGToDAGISel : public SelectionDAGISel {
public:
  SNESDAGToDAGISel(SNESTargetMachine &TM, CodeGenOpt::Level OptLevel)
      : SelectionDAGISel(TM, OptLevel), Subtarget(nullptr) {}

  StringRef getPassName() const override {
    return "SNES DAG->DAG Instruction Selection";
  }
...
bool SNESDAGToDAGISel::runOnMachineFunction(MachineFunction &MF) {
  Subtarget = &MF.getSubtarget<SNESSubtarget>();
  return SelectionDAGISel::runOnMachineFunction(MF);
}

bool SNESDAGToDAGISel::SelectAddr(SDNode *Op, SDValue N, SDValue &Base,
                                 SDValue &Disp) {
  SDLoc dl(Op);
...
bool SNESDAGToDAGISel::selectIndexedLoad(SDNode *N) {
  ...
  case MVT::i8: {
    ...
    Opcode = (isPre) ? SNES::LDRdPtrPd : SNES::LDRdPtrPi;
    break;
  }
  case MVT::i16: {
    ...
    Opcode = (isPre) ? SNES::LDWRdPtrPd : SNES::LDWRdPtrPi;
    break;
  }
  default:
    return false;
...
unsigned SNESDAGToDAGISel::selectIndexedProgMemLoad(const LoadSDNode *LD,
                                                   MVT VT) {
  ISD::MemIndexedMode AM = LD->getAddressingMode();
...
bool SNESDAGToDAGISel::SelectInlineAsmMemoryOperand(const SDValue &Op,
...
template <> bool SNESDAGToDAGISel::select<ISD::FrameIndex>(SDNode *N) {
  auto DL = CurDAG->getDataLayout();
  ...
  CurDAG->SelectNodeTo(N, SNES::FRMIDX,
                       getTargetLowering()->getPointerTy(DL), TFI,
                       CurDAG->getTargetConstant(0, SDLoc(N), MVT::i16));
  return true;
}

template <> bool SNESDAGToDAGISel::select<ISD::STORE>(SDNode *N) {
  ...
  unsigned Opc = (VT == MVT::i16) ? SNES::STDWSPQRr : SNES::STDSPQRr;
...
template <> bool SNESDAGToDAGISel::select<ISD::LOAD>(SDNode *N) {
  const LoadSDNode *LD = cast<LoadSDNode>(N);
  ...
  if (unsigned LPMOpc = selectIndexedProgMemLoad(LD, VT)) {
    ...
    switch (VT.SimpleTy) {
    case MVT::i8:
      ResNode = CurDAG->getMachineNode(SNES::LPMRdZ, DL, MVT::i8, MVT::Other,
                                       Ptr, RegZ);
      break;
    case MVT::i16:
      ResNode = CurDAG->getMachineNode(SNES::LPMWRdZ, DL, MVT::i16,
                                       MVT::Other, Ptr, RegZ);
      ReplaceUses(SDValue(N, 1), SDValue(ResNode, 1));
      break;
    default:
...
template <> bool SNESDAGToDAGISel::select<SNESISD::CALL>(SDNode *N) {
  SDValue InFlag;
  ...
  SDNode *ResNode =
    CurDAG->getMachineNode(SNES::ICALL, DL, MVT::Other, MVT::Glue, Ops);
...
template <> bool SNESDAGToDAGISel::select<ISD::BRIND>(SDNode *N) {
  SDValue Chain = N->getOperand(0);
...
bool SNESDAGToDAGISel::selectMultiplication(llvm::SDNode *N) {
  SDLoc DL(N);
  ...
  unsigned MachineOp = isSigned ? SNES::MULSRdRr : SNES::MULRdRr;
...
bool SNESDAGToDAGISel::trySelect(SDNode *N) {
  ...
  case SNESISD::CALL: return select<SNESISD::CALL>(N);
...
FunctionPass *createSNESISelDag(SNESTargetMachine &TM,
  ...
  return new SNESDAGToDAGISel(TM, OptLevel);
}
